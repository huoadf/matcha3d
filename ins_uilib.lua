...
local function safeReadGlobal(key)
    ...
    pcall(function() v = shared[key] end)
    return v
end
...
local function rect(x, y, w, h, color, z, radius, alpha)
    ...
    if HAS_CORNER then d.Corner = (radius or 0) * (ProjectState.roundScale or 1) end
    d.ZIndex = zord(z)
...
local function tri(a, b, c, color, z, filled, alpha)
    ...
    d.Filled = filled ~= false; d.Thickness = 1; d.ZIndex = zord(z)
    d.Transparency = (alpha or 1)
end
...
local function txt(value, x, y, color, size, font, z, centered, outline, maxW, alpha)
    ...
    d.Text = value; d.Color = (color == WHITE) and Theme.text or color
    d.Font = rf
    d.Size = size or 13
    if HAS_OUTLINE then d.Outline = outline == true end
    local xPos, yPos = x, y

    if centered == true then
        xPos = x - textWidth(value, size or 13, rf) / 2
        d.Center = false
    else
        d.Center = false
    end
    d.Position = v2(xPos, yPos)
    d.ZIndex = zord((z or 1) + 10)
    d.Transparency = (alpha or 1)
end
...
local function txtC(value, cx, y, color, size, font, z, alpha)
    ...
    local d = getDrawing("tx"); if not d then return end
    d.Text = value; d.Color = (color == WHITE) and Theme.text or color
    d.Font = resolveFont(font or FontSystem)
    d.Size = size or 13
    if HAS_OUTLINE then d.Outline = false end
    d.Center = true
    d.Position = v2(cx, y)
    d.ZIndex = zord((z or 1) + 10)
    d.Transparency = (alpha or 1)
...
local function gradRectH(x, y, w, h, c1, c2, z, alpha)
    ...
    for i = 0, steps - 1 do
        local t = i / (steps - 1)
        local col = Color3.new(c1.R + (c2.R - c1.R) * t, c1.G + (c2.G - c1.G) * t, c1.B + (c2.B - c1.B) * t)
        rect(x + sw * i, y, sw + 1, h, col, z, 0, alpha)
    end
end
    ...
    if item.type == "checkbox" then
        function handle:AddKeybind(defaultKey, mode, callback)
            local kb = { value = normalizeKey(defaultKey), mode = normalizeMode(mode),
                         callback = callback, listening = false }
            item.keybind = kb
            ...
            function kh:IsActivated() return item.value == true end
...
local function drawEditable(obj, value, fx, yText, size, color, alpha, z, fieldW, focused, caretIdx, selA)
    ...
    if focused and not hasSel then
        local ci = clamp(caretIdx - scroll, 0, #vis)
        local caretCh = (((clock() or 0) % 1) < 0.55) and "|" or " "
        vis = string.sub(vis, 1, ci) .. caretCh .. string.sub(vis, ci + 1)
    end
    if #vis > 0 then
        txt(vis, fx, yText, color, size, EDIT_FONT, z, false, false, nil, alpha)
    end
    ...
    if hasSel then
        ...
        rect(fx + vlo * cw, yText - 1, max(1, (vhi - vlo) * cw), size + 4, lerpColor(Theme.accentA, Theme.accentB, 0.5), z, 3, (vhi > vlo) and (0.45 * alpha) or 0)
    end
end
...
local function drawDropdown(click, rightClick)
    ...
    local accentMid = lerpColor(Theme.accentA, Theme.accentB, 0.5)
...
local function drawHotkeyOverlay(click, held)
    ...
    for _, item in ipairs(keybindItems) do
        ...
        if item._hkA > 0.02 then
            ...
            rows[#rows + 1] = { label = lbl, key = string.upper(kb and kb.value or ""), ra = item._hkA }
        end
    end
    local rowsH = 0
        ...
        if box.visible ~= false and box.alive ~= false then
            ...
            for i, ln in ipairs(lines) do
                ...
                elseif ln.kind == "bar" then
                    ...
                    if ln.pct > 0 then gradRectH(x + 14, ry + 5, (w - 28) * ln.pct / 100, 6, Theme.accentA, Theme.accentB, 163, 0.95) end
                else
...
local function drawWidget(item, rowX, rowY, rowW, trans, click, rightClick, popupBlocking)
    ...
    elseif t == "checkbox" then
        ...
        local onCol = item.risk and (Theme.unsafe or c3(255, 190, 70)) or lerpColor(Theme.accentA, Theme.accentB, 0.5)
        ...
        if item.keybind then
            local kb = item.keybind
            local lbl = kb.listening and "..." or string.upper(kb.value or "none")
            ...
            if kbHov and not kb.listening then tooltipReq("click rebind  \194\183  right-click mode", ProjectState.mouseX, ProjectState.mouseY) end
            if click and kbHov then kb.listening = true; click = false end
            if rightClick and kbHov then
            ...
            if item.keybind.listening then
                for _, name in ipairs(InputOrder) do
                    local inp = Input[name]
                    if inp.click and name ~= "m1" and name ~= "m2" then
    ...
    elseif t == "keybind" then
        local lbl = item.listening and "..." or string.upper(item.value or "none")
        ...
        if item.tooltip and interact and over(rowX, rowY, rowW, 26) then tooltipReq(item.tooltip, ProjectState.mouseX, ProjectState.mouseY) end
        if click and kbHov then item.listening = true; ProjectState.kbCapture = item; click = false end
        if item.listening then
            for _, name in ipairs(InputOrder) do
                local inp = Input[name]
                if inp.click and name ~= "m1" and name ~= "m2" then
    ...
    elseif t == "slider" then
        ...
        if item._dispVal ~= item.value then
            ...
            item._dispStr = tostring(item.value) .. (item.suffix ~= "" and (" " .. item.suffix) or "")
        end
    ...
    elseif t == "rangeslider" then
        ...
        local disp = tostring(item.valueLo) .. " - " .. tostring(item.valueHi) .. (item.suffix ~= "" and (" " .. item.suffix) or "")
...
local function drawSectionCard(section, colX, sy, colW, secH, clipTop, clipBottom, click, held, rightClick, popupBlocking)
    ...
    if headerH > 0 then
        ...
        if ha > 0.01 then
            local cc = lerpColor(Theme.accentA, Theme.accentB, 0.5)
            txt(string.upper(section.name), colX + 4, sy + 2, cc, 11, FontBold, 31, false, false, colW - 22, 0.85 * ha)
    ...
    if drawH <= 0 then return click, rightClick end
    ...
    local cardAcc = lerpColor(Theme.accentA, Theme.accentB, 0.5)
    ...
    for i, item in ipairs(section.items) do
        ...
        if rowY + ih > clipTop and rowY < cardBottom then
            ...
            strokeRect(rowX - 4, rowY - 4, rowW + 8, ih + 8, lerpColor(Theme.accentA, Theme.accentB, 0.5), 33, 7, fa * trans)
        end
...
local function drawWindow(click, held, rightClick)
    ...
    local gemX = x + 20
    local accentMidT = lerpColor(Theme.accentA, Theme.accentB, 0.5)
    ...
    do
        ...
        for _, d in ipairs({ { 1.4, 0 }, { -1.4, 0 }, { 0, 1.4 }, { 0, -1.4 } }) do
            txt(ProjectState.title, hdrX + d[1], tTop + d[2], Theme.accentA, 16, FontBold, 60, false, false, twMax, 0.26 * tA)
        end
        txt(ProjectState.title, hdrX, tTop, lerpColor(Theme.accentA, Theme.accentB, 0.5), 16, FontBold, 61, false, false, twMax, tA)
    end
    ...
    for i, tab in ipairs(ProjectState.tabs) do
        ...
        local hov = over(x + 12, ty, sw - 24, pillH) and ProjectState.hoverEffects ~= false
        local af = approach(tab._af or 0, active and 1 or 0, 13); tab._af = af
        rect(x + 12, ty, sw - 24, pillH, lerpColor(Theme.accentA, Theme.accentB, 0.5), 61, 4, 0.13 * af * v)
    ...
    do
        ...
        if ProjectState.avatarImg then
            ...
            circ(acx, acy, 14, lerpColor(Theme.accentA, Theme.accentB, 0.5), 61, true, 1, 28, 0.18 * v)
        ...
        if ProjectState.avatarImg then
            ...
            circ(acx, acy, 11, lerpColor(Theme.accentA, Theme.accentB, 0.5), 13, true, 1, 24, 0.18 * v)
        ...
        for i, tab in ipairs(ProjectState.tabs) do
            if not tab.hidden then
                ...
                local ph = 26
                local pyy = sMid - ph / 2
                local hov = over(tx, pyy, pw0, ph) and ProjectState.hoverEffects ~= false
                local af = approach(tab._af or 0, active and 1 or 0, 13); tab._af = af
    ...
    while i <= #notes do
        ...
        if n.elapsed >= n.duration then remove(notes, i)
            ...
            local tc = (typ == "success" and c3(95, 210, 135)) or (typ == "warning" and c3(255, 190, 70))
                       or (typ == "error" and Theme.bad) or lerpColor(Theme.accentA, Theme.accentB, 0.5)
...
local CFG_FOLDER = LIB_NAME .. "_configs"
local function ensureFolder() pcall(function() if not isfolder(CFG_FOLDER) then makefolder(CFG_FOLDER) end end) end
local function serializeConfig()
    ...
    for _, tab in ipairs(ProjectState.tabs) do
        for _, sec in ipairs(tab.sections) do
            for _, item in ipairs(sec.items) do
                if item.value ~= nil and item.type ~= "button" then
...
function ui:SaveConfig(name)
    ...
    if enc and writefile then pcall(writefile, CFG_FOLDER .. "/" .. name .. ".json", enc) end
    return self
end
local function applyConfigData(data)
    ...
    for _, tab in ipairs(ProjectState.tabs) do
        for _, sec in ipairs(tab.sections) do
            for _, item in ipairs(sec.items) do
                if item.value ~= nil and item.label and item.type ~= "button" then
                    local key = tab.name .. "." .. sec.name .. "." .. item.label
...
function ui:LoadConfig(name)
    ...
    applyConfigData(raw and jsonDecode(raw))
    return self
end

local AUTOLOAD_FILE = CFG_FOLDER .. "/_autoload.json"
local function readAutoloadPrefs()
    if not (isfile and isfile(AUTOLOAD_FILE) and readfile) then return {} end
    local raw; pcall(function() raw = readfile(AUTOLOAD_FILE) end)
...
local function writeAutoloadPref(base, name)
    ...
    ensureFolder(); local enc = jsonEncode(t); if enc and writefile then pcall(writefile, AUTOLOAD_FILE, enc) end
end
...
function ui:DeleteConfig(name)
    ...
    if name ~= "" and delfile then pcall(delfile, CFG_FOLDER .. "/" .. name .. ".json") end
    return self
end
...
function ui:SetLogo(src)
    ...
    local target = tostring(src); ProjectState.logoSrc = target
    local s1, s2 = string.byte(target, 1, 2)
    if #target > 24 and ((s1 == 0x89 and s2 == 0x50) or (s1 == 0xFF and s2 == 0xD8)) then
        pcall(function() local img = Drawing.new("Image"); img.Data = target; ProjectState.logoImg = img end)
        return self
    end
    task.spawn(function()
        pcall(function()
            ...
            if b and #b > 12 then
                local b1, b2 = string.byte(b, 1, 2)
                if (b1 == 0x89 and b2 == 0x50) or (b1 == 0xFF and b2 == 0xD8) then
                    if ProjectState.logoSrc == target then local img = Drawing.new("Image"); img.Data = b; ProjectState.logoImg = img end
                end
            end
        end)
    end)
...
function ui:SetIcon(src)   -- the corner gem + minimized bubble icon (separate from the header logo)
    ...
    local target = tostring(src); ProjectState.iconSrc = target
    local s1, s2 = string.byte(target, 1, 2)
    if #target > 24 and ((s1 == 0x89 and s2 == 0x50) or (s1 == 0xFF and s2 == 0xD8)) then
        pcall(function() local img = Drawing.new("Image"); img.Data = target; ProjectState.iconImg = img end)
        return self
    end
    task.spawn(function()
        pcall(function()
            ...
            if b and #b > 12 then
                local b1, b2 = string.byte(b, 1, 2)
                if (b1 == 0x89 and b2 == 0x50) or (b1 == 0xFF and b2 == 0xD8) then
                    if ProjectState.iconSrc == target then local img = Drawing.new("Image"); img.Data = b; ProjectState.iconImg = img end
                end
            end
        end)
    end)
...
local function step()
    ...
    drawNotifications()
    drawDialog(dlgClick)
    if (ProjectState.minA or 0) < 0.06 then local r = ProjectState._winRect; if r then drawBgEffect(r.x, r.y, r.w, r.h, r.th, r.v) end end
    hideUnused()
end
...
task.spawn(function()
    while ProjectState.alive do
        runStepSafe()
        if ProjectState.autoSave and ((clock() or 0) - (ProjectState._autoSaveT or 0)) > 1.2 then
            ProjectState._autoSaveT = clock() or 0
            pcall(function()
                local enc = jsonEncode(serializeConfig())
                if enc and enc ~= ProjectState._lastCfgEnc then
                    ProjectState._lastCfgEnc = enc; ensureFolder()
                    if writefile then writefile(CFG_FOLDER .. "/" .. tostring(ProjectState.configName or "default") .. ".json", enc) end
                end
            end)
        end
...
local function buildSettingsTab(win, icon)
    ...
    presetDrop = th:Dropdown("Preset", { "Custom" }, presetChoices, false, function(v)
        if v[1] and v[1] ~= "Custom" then
            local p = ThemePresets[v[1]]
            if p then
                applyAccents(p[1], p[2])
                applyThemeExtras(v[1])
                if c1pick then c1pick.item.value = p[1] end
                if c2pick then c2pick.item.value = p[2] end
            end
        end
    end, "preset palettes; picking a colour below switches this to Custom", true)
    c1pick = th:Colorpicker("Color 1", Theme.accentA, function(c) applyAccents(c, ProjectState.baseAccentB); if presetDrop then presetDrop:Set({ "Custom" }) end end)
    c2pick = th:Colorpicker("Color 2", Theme.accentB, function(c) applyAccents(ProjectState.baseAccentA, c); if presetDrop then presetDrop:Set({ "Custom" }) end end)
    ...
    local saved
    local function refresh() if saved then saved:UpdateChoices(ui:ListConfigs()) end end
    cf:Button("Save config", function() ui:SaveConfig(ProjectState.configName); refresh(); ui:Notify("config", "saved to " .. CFG_FOLDER .. "/" .. tostring(ProjectState.configName) .. ".json", 5) end)
    cf:Button("Load config", function() ui:LoadConfig(ProjectState.configName); ui:Notify("config", "loaded: " .. tostring(ProjectState.configName), 3) end)
    cf:Button("Delete config", function() ui:DeleteConfig(ProjectState.configName); refresh(); ui:Notify("config", "deleted", 3) end)
    saved = cf:Dropdown("Saved", {}, ui:ListConfigs(), false, function(v) if v[1] then ProjectState.configName = v[1]; nameBox:Set(v[1]) end end, nil, true)
    cf:Toggle("Auto-load on launch", readAutoloadPref(ProjectState._baseConfigName) ~= nil, function(on)
        if on then writeAutoloadPref(ProjectState._baseConfigName, ProjectState.configName); ui:SetAutoSave(true); ui:Notify("config", "auto-load: " .. tostring(ProjectState.configName), 3)
        else writeAutoloadPref(ProjectState._baseConfigName, nil); ui:SetAutoSave(false); ui:Notify("config", "auto-load off", 2) end
    end, "load the selected config every launch and keep saving changes to it")
...
function ui:CreateWindow(config)
    ...
    function win:AddSettingsTab(icon) return buildSettingsTab(win, icon) end
    ...
    task.spawn(function()
        ...
        local function setImg(b)
            ...
            local img
            local ok = pcall(function() img = Drawing.new("Image"); img.Data = b end)
    ...
    task.spawn(function()
        ...
        if pref then ProjectState.configName = pref; ProjectState.autoSave = true end
        if ProjectState.autoSave then
